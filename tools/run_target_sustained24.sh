...
run_id="${1:?usage: run_target_sustained24.sh RUN_ID}"
...
systemd-run --user --scope --property=MemoryMax=18G --property=MemorySwapMax=0 \
  ...
  --backend streaming \
  --variant gpu8-stream24-direct-no-repack-ub32-greedy256-sustained \
  --workload workloads/m5_sustained.txt \
  --cache-condition new-process-direct-experts-existing-system-cache-context-reused \
  --timeout-s 1800 --max-rss-gib 17 --min-available-gib 6 --max-gpu-mib 7000 \
  --env LLAMA_MOE_STREAM_NO_PRELOAD=1 \
  --stdin-file workloads/m5_sustained.txt \
