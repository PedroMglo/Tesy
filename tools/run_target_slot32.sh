...
run_id="${1:?usage: run_target_slot32.sh RUN_ID}"
...
systemd-run --user --scope --property=MemoryMax=18G --property=MemorySwapMax=0 \
  ...
  --backend streaming \
  --variant gpu8-stream32-direct-no-repack-ub32-greedy256 \
  ...
  -lv 3 --moe-stream-cache 32s --moe-stream-io-threads 4 --moe-stream-direct
