...
run_id="${1:?usage: run_target_two_requests.sh RUN_ID}"
...
systemd-run --user --scope --property=MemoryMax=18G --property=MemorySwapMax=0 \
  ...
  --backend streaming \
  --variant gpu8-stream16-direct-no-repack-ub32-greedy256 \
  --workload workloads/m1_two_turns.txt \
  --cache-condition cold-process-direct-experts-after-prefix-probes \
  ...
  -lv 3 --moe-stream-cache 16s --moe-stream-io-threads 4 --moe-stream-direct
