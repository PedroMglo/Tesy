#!/usr/bin/env python3
"""Compare paired frozen core8 task outcomes without causal parameter-count claims."""
...
def compare(stock_grade, target_grade, stock_raw, target_raw):
    ...
    for i in range(2):
        if grades[i]["schema_version"]!="c2-task-grade-v2" or \
           [x["task_id"] for x in grades[i]["scores"]]!=list(CORE_IDS) or \
           raw[i]["preflight"]["config"]["suite"]!="c2core8" or \
           [x["id"] for x in raw[i]["results"]]!=list(CORE_IDS) or \
           raw[i]["stop_reasons"] or raw[i]["returncode"]!=0:
            raise GateError("incomplete or non-core8 run/grade")
    ...
    for i,task_id in enumerate(CORE_IDS):
    ...
    return {"schema_version":"c2-core8-comparison-v1","n":8,"counts":counts,
            ...
            "limitations":["eight-task pilot selected before responses; no significance claim",
