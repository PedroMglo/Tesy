...
- for every requested `N>0`, all six merged gpt-oss expert tensors
  ...
  explicit loader override generated by the pinned `--n-cpu-moe` rule; the
  selected compatible buffer type is retained verbatim and is not inferred
  from its name;
...
The placement gate proves the explicit per-tensor override emitted by the
pinned loader and records the concrete selected buffer type. The loader may
choose a compatible buffer from its CPU buffer list (including extra/repack or
host-buffer candidates) rather than the literal CPU default. The gate does not
infer realized buffer identity from the requested flag alone.
