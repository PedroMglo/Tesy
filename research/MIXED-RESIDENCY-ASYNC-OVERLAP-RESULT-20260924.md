...
The next discriminating gate is the cheaper, separately prospective 81-pair order-conditioned steady-tail diagnostic for h=2/h=3. It must distinguish serial→async transition artifacts from steady async tail behavior before routed-layer/backend integration is authorized. Integration remains contingent on that tail gate, with resource/provenance telemetry preserved. No prefetch work is authorized by this isolated result alone.
