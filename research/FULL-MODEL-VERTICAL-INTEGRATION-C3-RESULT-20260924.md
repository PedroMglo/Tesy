...
O resumo compacto por run e agregados está no Git em `research/coordination/C3-STOCK-EVIDENCE.json`; o auditor `src/tesy/full_stock_eval_audit.py` recalcula-o dos raw locais. O `summary-final.json` local tem SHA-256 `47d63e066c45bd52f03302b9e19fce1f14de5b2e39847bfc66a8555c3830fbff`. Raw SSE, arrivals, outputs, doctor, telemetria, provenance por processo, vector de logits do blocker e binários ficam **apenas locais**; hashes sem ficheiros acessíveis não permitem auditoria remota completa. O endpoint SSE não expõe vector completo de token IDs; publicamos texto e contagens, não fingimos sequência exacta de IDs. A auditoria externa deve obter esses artefactos por canal apropriado se os exigir.
