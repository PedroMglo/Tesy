...
| mixed GPU-hit / CPU-miss expert execution | ROUTED_LAYER_EXACTNESS_GO_CB_EVAL_BOUND_IMPLEMENTED_NOT_RUN | Physical isolated async median/tail gates and one-event routed exactness PASS. Source audit closes scheduler-node expert subset placement and current-pin meta expert-axis split with global IDs (`STATIC_NO_GO`); a 81-pair zero-Tesy-work `cb_eval` cancellation lower bound is implemented prospectively to test external live-router interception overhead before any handoff/timing patch. Physical bound NOT_RUN. |
