...
| mixed GPU-hit / CPU-miss expert execution | ROUTED_LAYER_EXACTNESS_GO | Physical isolated async median and 81-pair steady-tail gates PASS. Physical routed exactness campaign `routed-layer-exactness-20260924T165550Z` then reproduced one real stock layer-0 decode event: OFF/ON greedy token IDs `[2167,1309]`, experts `[1,13,17,21]`, final softmax routing weights, and h=2/h=3 serial+async numerical parity vs stock (`relative_max=5.60445568e-08`, cosine 1.0). This is one-event correctness evidence, not routed-layer timing or full-model speedup. |
