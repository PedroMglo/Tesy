...
| mixed GPU-hit / CPU-miss expert execution | CB_EVAL_INTERCEPT_BOUND_SURVIVES_LIVE_HANDOFF_EXACTNESS_IMPLEMENTED_NOT_RUN | Physical isolated async median/tail, one-event routed exactness and corrected zero-Tesy-work cb_eval bound PASS/SURVIVES. Next gate is implemented correctness-only in one process: stock-reference arm → KV rollback → live activation/top-k/final-weight early-stop arm → KV rollback → Tesy h=2/h=3 serial+async FFN vs stock `ffn_moe_out`. Numerical parity is gating; activation bitwise equality is diagnostic. Physical handoff exactness NOT_RUN; no timing/reinjection/prefetch claim. |
