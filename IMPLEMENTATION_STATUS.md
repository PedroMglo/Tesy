...
| mixed GPU-hit / CPU-miss expert execution | LIVE_MOE_HANDOFF_TIMING_SERIAL_PIVOT | Physical live resident-expert timing campaign `results/live-moe-handoff-timing-20260924T194737Z/` passed all provenance, resource, 81-sample/order-balance and pre/post exactness gates for h=2/h=3. Async beats callback-instrumented stock but fails retained-overlap versus serial; serial beats stock in both windows and is selected. The published headroom is a diagnostic ceiling only; no reinjection, committed-token, cache-miss, prefetch, physical-traffic or end-to-end claim. |
