...
| mixed GPU-hit / CPU-miss expert execution | LIVE_MOE_HANDOFF_EXACTNESS_GO_TIMING_IMPLEMENTED_NOT_RUN | Physical isolated async median/tail, routed exactness, cb_eval lower bound and in-process live handoff exactness passed. The next gate is prospectively implemented resident-expert live timing in separate h=2/h=3 processes: pre/post exactness, 6 warmup + 81 balanced stock/serial/async triplets, activation-ready and route-ready windows, provenance start gate, independent statistics/decision, and diagnostic reinjection-headroom derivation. Physical timing NOT_RUN; no reinjection, committed-token, cache-miss, prefetch or end-to-end claim. |
