...
| mixed GPU-hit / CPU-miss expert execution | LIVE_MOE_HANDOFF_EXACTNESS_GO | Physical isolated async median/tail, one-event routed exactness and corrected zero-Tesy-work cb_eval bound PASS/SURVIVES. Physical in-process live handoff exactness campaign `live-moe-handoff-exactness-20260924T180016Z` then PASSed: stock-reference and handoff activation bitwise equal; token `2167`; experts `[1,13,17,21]`; h=2/h=3 serial-vs-stock, async-vs-stock and async-vs-serial PASS with max relative error `5.60445568e-08`; zero swap. This authorizes only a separately frozen timing protocol for the same live boundary; no reinjection/prefetch/end-to-end claim. |
