...
| mixed GPU-hit / CPU-miss expert execution | ASYNC_STEADY_TAIL_GO_ROUTED_EXACTNESS_IMPLEMENTED_NOT_RUN | Physical isolated async median gate PASS and 81-pair steady-tail gate PASS (`ASYNC_STEADY_TAIL_GO`). Next gate captures one real stock layer-0 decode activation/top-k/final routing weights/MoE output and checks h=2/h=3 serial+async numerical parity against stock before any routed-layer timing. Exactness campaign NOT_RUN. |
