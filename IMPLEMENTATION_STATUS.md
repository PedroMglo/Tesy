...
| mixed GPU-hit / CPU-miss expert execution | LIVE_MOE_HANDOFF_TIMING_SERIAL_PIVOT | A campanha física live resident-expert `results/live-moe-handoff-timing-20260924T194737Z/` passou todas as gates de proveniência, recursos, 81 amostras/balanceamento ordinal e exactness pré/pós para h=2/h=3. Async bate stock callback-instrumented mas não retém overlap contra serial; serial bate stock nas duas janelas e é seleccionado. A folga publicada é apenas ceiling diagnóstico; não há claim de reinjection, committed-token, cache-miss, prefetch, tráfego físico ou end-to-end. |
| post-compute live MoE reinjection + committed continuation | LIVE_MOE_REINJECTION_CONTINUATION_EXACTNESS_GO | Campanha física `results/live-moe-reinjection-continuation-exactness-20260924T201414Z/`: stock/control/serial h2/serial h3 com contextos frescos produziram tokens 2167/1309/316; control e h2/h3 passaram full-logits parity nos checkpoints A/B e o decode normal adicional de 1309. FFN Tesy vs stock relative max 5.60445568e-08. Proveniência, recursos e zero swap PASS. O stock MoE continua a calcular; não há claim de skip, speedup, end-to-end, cache, prefetch ou tráfego físico. |
| CPU split graph layer-0 stock MoE skip + committed continuation | LIVE_MOE_SPLIT_GRAPH_SKIP_EXACTNESS_GO | Campanha física `results/live-moe-split-graph-skip-exactness-20260924T205335Z/`: stock, patched null-hook, segmented same-work, skip-stock e serial Tesy h2/h3 passaram. Graph real 1,349 nodes CPU; middle `[40,56)` inclui 3 `MUL_MAT_ID` e teve compute count 0 nos skip arms. FFN h2/h3 relative max 5.604455676769968e-08; full logits A/B bitwise iguais e tokens 2167/1309/316 com continuação committed. Proveniência/recursos PASS, zero swap. Apenas correctness deste evento; sem performance, outras rotas/layers, cache ou prefetch. |
| corrected Tesy GPU FFN aggregation | MODEL_FREE_TOPOLOGY_EXECUTION_PASS | Agregação leaf-only recebe parcial GPU por cópia explícita; teste nativo prova FFN GPU executado uma vez. Timings anteriores não descrevem este caminho corrigido. |
| full-model stock chat baseline | MEASURED_STOCK_ONLY_20_RUNS | GGUF/template incorporado, 2 tarefas públicas, 5 processos por tarefa/colocação, `n_gpu_layers=0/12`, EOS preservado, provenance/runtime/resources PASS e zero swap; medianas/p95 e runs em `research/FULL-MODEL-VERTICAL-INTEGRATION-C3-RESULT-20260924.md`. Apenas stock, sem comparação Tesy. |
| live variable-route Tesy vertical candidate | NUMERICAL_PARITY_BLOCKER | Residency fixa GPU IDs `{0,1,2,3}` por layer. Duas layers passam no caso diagnóstico; com a terceira, FFN h=1 diverge e full-logit relative max `0,00855978666` excede `0,005`. Root preservada; 24 layers, timed path, negative control e end-to-end Tesy NOT_RUN. |
| larger-model admission report | INSTALLED_MEASURED_CANDIDATES_ESTIMATED | Installed GGUF e recursos observados; Qwen3-30B/gpt-oss-120b sem artifact exacto/admissão. Loader >RAM, physical I/O e tok/s maiores DESCONHECIDO; nenhum modelo maior descarregado. |
