...
| mixed GPU-hit / CPU-miss expert execution | ASYNC_OVERLAP_MEASURED_GO_TAIL_ORDER_NOT_RUN | Physical isolated async campaign PASS with `ASYNC_OVERLAP_MEASURED_GO` (13.15% weighted-median reduction). Post-hoc sample-order analysis localizes h=2/h=3 aggregate tail spikes to serial→async transition buckets; 81-sample order-conditioned steady-tail gate is frozen and NOT_RUN before routed-layer integration. |
