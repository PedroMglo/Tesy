...
| mixed GPU-hit / CPU-miss expert execution | OVERLAP_BOUND_PASS_ASYNC_IMPLEMENTED_NOT_RUN | Physical overlap-bound campaign PASS with `OVERLAP_IMPLEMENTATION_GO`; measured async candidate now implemented prospectively for h=1..3 with same-campaign serial baseline and frozen 10% gate. Async physical campaign NOT_RUN. |
