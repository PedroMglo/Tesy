...
| mixed GPU-hit / CPU-miss expert execution | CB_EVAL_INTERCEPT_BOUND_SURVIVES | Physical isolated async median/tail and one-event routed exactness gates PASS. Source audit closes scheduler-node expert subset placement and current-pin meta expert-axis split with global IDs (`STATIC_NO_GO`). Corrected physical 81-pair zero-Tesy-work `cb_eval` early-stop bound survived: stock MoE 2.675066/2.873420 ms median/p95 vs early-stop 0.049904/0.057839 ms (ratios 0.018655/0.020129), zero swap. This only authorizes real activation handoff exactness; it is not routed-layer performance. |
